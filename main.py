...
REGION_LEADERS_ENV_PATH = (os.getenv("REGION_LEADERS_ENV_PATH") or "region_leaders.env").strip()
...
def create_fsm_storage():
    backend = FSM_STORAGE_BACKEND
    if backend not in {"auto", "memory", "mongo"}:
        logging.warning("FSM_STORAGE_BACKEND noto'g'ri berilgan: %s. 'auto' ishlatiladi.", backend)
        backend = "auto"

    if backend == "memory":
        logging.info("FSM storage sifatida MemoryStorage ishlatildi.")
        return MemoryStorage()

    is_vercel_runtime = bool(os.getenv("VERCEL"))
    use_mongo_storage = backend == "mongo" or (backend == "auto" and is_vercel_runtime)
    if not use_mongo_storage:
    ...
    try:
        ...
        logging.info(
            "FSM storage sifatida MongoDB ishlatildi. db=%s collection=%s",
            FSM_MONGO_DB_NAME,
            FSM_MONGO_COLLECTION_NAME,
        )
        return storage
    except Exception as error:
        if backend == "mongo":
...
def sanitize_region_token(region_name: str | None) -> str:
    value = str(region_name or "").upper()
    for char in ("'", "`"):
...
REGION_NAME_LOOKUP: dict[str, str] = {}
for region in QAMASHI_HUDUDLARI:
    REGION_NAME_LOOKUP[normalize_region_name(region)] = region
    sanitized_region = sanitize_region_token(region).casefold()
    if sanitized_region:
        REGION_NAME_LOOKUP.setdefault(sanitized_region, region)


def resolve_region_name(region_name: str | None) -> str | None:
    raw_region_name = str(region_name or "").strip()
    if not raw_region_name:
        return None

    normalized_name = normalize_region_name(raw_region_name)
    if normalized_name in REGION_NAME_LOOKUP:
        return REGION_NAME_LOOKUP[normalized_name]

    sanitized_name = sanitize_region_token(raw_region_name).casefold()
    if sanitized_name:
        return REGION_NAME_LOOKUP.get(sanitized_name)

    return None


def build_region_admin_env_key(region: str) -> str:
    return f"MFY_ADMIN_{sanitize_region_token(region)}"


def parse_region_leader_value(raw_value: str, source_label: str) -> tuple[int, str]:
    parts = [part.strip() for part in str(raw_value).split("|")]
    raw_user_id = parts[0] if parts else ""
    if not raw_user_id.isdigit():
        raise ValueError(
            f"{source_label} noto'g'ri formatda. Misol: "
            f"{source_label}=123456789 | @username"
        )
    ...
    return int(raw_user_id), contact_value


def load_region_leader_fallbacks(path: str) -> tuple[dict[str, int], dict[str, str]]:
    mappings: dict[str, int] = {}
    contact_urls: dict[str, str] = {}
    if not path or not os.path.exists(path):
        return mappings, contact_urls
    ...
    with open(path, "r", encoding="utf-8") as source_file:
        for line_number, raw_line in enumerate(source_file, start=1):
            ...
            if "=" not in line:
                raise ValueError(
                    f"{path}:{line_number} noto'g'ri formatda. "
                    "Misol: Azlartepa=123456789 | @azlartepa_admin"
                )

            raw_region, raw_value = line.split("=", maxsplit=1)
            canonical_region = resolve_region_name(raw_region)
            if not canonical_region:
                raise ValueError(
                    f"{path}:{line_number} dagi `{raw_region.strip()}` MFY bot ro'yxatida topilmadi."
                )

            if canonical_region in mappings:
                raise ValueError(
                    f"{path}:{line_number} dagi `{canonical_region}` MFY ikki marta berilgan."
                )

            user_id, contact_value = parse_region_leader_value(raw_value, f"{path}:{line_number}")
            mappings[canonical_region] = user_id
            if contact_value:
                contact_urls[canonical_region] = contact_value
...
def validate_region_leader_ids(
    ...
    if not mappings:
        raise ValueError(
            ...
            "`.env` yoki `region_leaders.env` ichida barcha MFYlarni kiriting."
        )
    ...
    has_duplicate_assignments = bool(duplicate_assignments)

    if ALLOW_SHARED_REGION_ADMINS and has_duplicate_assignments:
        duplicate_text = "; ".join(
            f"{user_id} -> {', '.join(regions)}"
            for user_id, regions in sorted(duplicate_assignments.items())
        )
        logging.warning(
            "ALLOW_SHARED_REGION_ADMINS yoqilgan. Bitta admin bir nechta MFYga biriktirildi: %s",
            duplicate_text,
        )
        duplicate_assignments = {}
    ...
    if missing_regions or duplicate_assignments:
        ...
        if missing_entries:
            errors.append(
                "To'ldirilmagan .env qatorlari (fallback ham topilmadi): "
...
def load_region_leader_configs() -> tuple[dict[str, int], dict[str, str]]:
    ...
    contact_urls: dict[str, str] = {}
    missing_entries: list[str] = []
    fallback_mappings, fallback_contact_urls = load_region_leader_fallbacks(REGION_LEADERS_ENV_PATH)
    fallback_regions_used: list[str] = []

    for region in QAMASHI_HUDUDLARI:
        env_key = build_region_admin_env_key(region)
        raw_value = (os.getenv(env_key) or "").strip()
        if not raw_value:
            fallback_user_id = fallback_mappings.get(region)
            if fallback_user_id is None:
                missing_entries.append(f"{region} -> {env_key}")
                continue

            mappings[region] = fallback_user_id
            fallback_contact = fallback_contact_urls.get(region)
            if fallback_contact:
                contact_urls[region] = fallback_contact
            fallback_regions_used.append(region)
            continue

        user_id, contact_value = parse_region_leader_value(raw_value, env_key)
        mappings[region] = user_id
        if contact_value:
            contact_urls[region] = contact_value
        elif region in fallback_contact_urls:
            contact_urls[region] = fallback_contact_urls[region]

    resolved_mappings = validate_region_leader_ids(mappings, missing_entries)
    if fallback_regions_used:
        logging.warning(
            ".env ichida topilmagan %s ta MFY admini `%s` faylidan olindi: %s",
            len(fallback_regions_used),
            REGION_LEADERS_ENV_PATH,
            ", ".join(fallback_regions_used),
        )

    return resolved_mappings, contact_urls
...
FINISH_ADDITIONAL_INFO_BUTTON_TEXT = "\U00002705 Davom etish"
BOT_CREATOR_CONTACT_TEXT = "Bot yaratish uchun boglanish: @livee100301"
PHOTO_GROUP_ATTACHMENT_TYPE = "photo_group"
...
def get_attachment_type_label(attachment_type: str | None) -> str:
    labels = {
        ...
        PHOTO_GROUP_ATTACHMENT_TYPE: "\U0001F5BC Rasmlar",
...
def serialize_attachment_file_ids(file_ids: list[str]) -> str:
    return json.dumps(file_ids, ensure_ascii=True)


def parse_attachment_file_ids(attachment_type: str | None, attachment_file_id: str | None) -> list[str]:
    if not attachment_file_id:
        return []

    normalized_type = str(attachment_type or "").strip().lower()
    if normalized_type != PHOTO_GROUP_ATTACHMENT_TYPE:
        return [str(attachment_file_id).strip()]

    try:
        parsed_value = json.loads(str(attachment_file_id))
    except json.JSONDecodeError:
        parsed_value = [attachment_file_id]

    if not isinstance(parsed_value, list):
        parsed_value = [attachment_file_id]

    return [str(file_id).strip() for file_id in parsed_value if str(file_id).strip()]


def get_attachment_item_count(attachment_type: str | None, attachment_file_id: str | None) -> int:
    return len(parse_attachment_file_ids(attachment_type, attachment_file_id))
...
def build_attachment_upload_text(section_key: str) -> str:
    ...
    return (
        ...
        "Bir yoki bir nechta rasm, yoki bitta video/hujjat yuborishingiz mumkin.\n"
        "Rasmlarni yuborib bo'lgach, pastdagi `Davom etish` tugmasini bosing. \U00002705"
    )


def build_attachment_saved_notice(
    attachment_type: str | None,
    attachment_file_id: str | None = None,
) -> str:
    attachment_label = get_attachment_type_label(attachment_type)
    if str(attachment_type or "").strip().lower() == PHOTO_GROUP_ATTACHMENT_TYPE:
        attachment_count = get_attachment_item_count(attachment_type, attachment_file_id)
        attachment_label = f"{attachment_count} ta rasm" if attachment_count else attachment_label

    return (
        f"\U00002705 Qo'shimcha ma'lumot saqlandi: {attachment_label}\n\n"
...
def build_message_with_attachment_note(
    message_text: str,
    attachment_type: str | None,
    attachment_file_id: str | None = None,
) -> str:
    ...
    return f"{cleaned_text}\n\n[\U0001F4CE Qo'shimcha ma'lumot biriktirilgan: {attachment_label}]"


def build_attachment_admin_note(
    attachment_type: str | None,
    attachment_file_id: str | None = None,
    ...
    note = f"\n\n\U0001F517 {attachment_label} pastdagi tugma orqali ochiladi."
    if attachment_caption:
        note += f"\n\U0001F4DD Qo'shimcha izoh: {attachment_caption.strip()}"
    return note


def append_bot_creator_contact(text: str) -> str:
    return f"{text}\n\n{BOT_CREATOR_CONTACT_TEXT}"
...
async def send_request_attachment_on_demand(
    ...
    if attachment_type == PHOTO_GROUP_ATTACHMENT_TYPE:
        photo_file_ids = parse_attachment_file_ids(attachment_type, attachment_file_id)
        if not photo_file_ids:
            await bot.send_message(chat_id=chat_id, text=localize_text_for_user(chat_id, "Rasmlarni ochib bo'lmadi."))
            return
        ...
        caption_sent = False
        for start_index in range(0, len(photo_file_ids), 10):
            media = []
            for photo_file_id in photo_file_ids[start_index : start_index + 10]:
                media.append(
                    InputMediaPhoto(
                        media=photo_file_id,
                        caption=caption if not caption_sent else None,
                    )
                )
                caption_sent = True
...
def build_attachment_upload_keyboard(script: str = DEFAULT_SCRIPT) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=localize_text_for_script(script, FINISH_ADDITIONAL_INFO_BUTTON_TEXT),
                    callback_data="attachment_choice:continue",
                )
            ],
        ]
    )
...
async def ask_full_name(message: Message, state: FSMContext, user_id: int | None = None) -> None:
    ...
    text = append_bot_creator_contact(
        (
            "Assalomu alaykum! \U0001F44B\n\n"
...
async def attachment_choice_handler(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if attachment_type and attachment_file_id:
        intro_text = build_attachment_saved_notice(str(attachment_type), str(attachment_file_id))

    await show_section_text_prompt(call.message, state, section_key, user_id=user_id, intro_text=intro_text)
...
async def attachment_upload_handler(message: Message, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    existing_attachment_type = str(data.get("attachment_type") or "").strip().lower()
    existing_attachment_file_id = data.get("attachment_file_id")
    ...
    if message.photo:
        photo_file_ids = (
            parse_attachment_file_ids(existing_attachment_type, str(existing_attachment_file_id))
            if existing_attachment_type == PHOTO_GROUP_ATTACHMENT_TYPE and existing_attachment_file_id
            else []
        )
        photo_file_ids.append(message.photo[-1].file_id)
        attachment_type = PHOTO_GROUP_ATTACHMENT_TYPE
        attachment_file_id = serialize_attachment_file_ids(photo_file_ids)
        attachment_caption = (message.caption or "").strip() or data.get("attachment_caption")
        await state.update_data(
            attachment_type=attachment_type,
            attachment_file_id=attachment_file_id,
        ...
        if not message.media_group_id:
            await message.answer(
                localize_text_for_user(
                    user_id,
                    (
                        f"\U00002705 {len(photo_file_ids)} ta rasm saqlandi.\n\n"
                        "Yana rasm yuborishingiz yoki pastdagi `Davom etish` tugmasini bosishingiz mumkin."
                    ),
                ),
        ...
        if existing_attachment_type == PHOTO_GROUP_ATTACHMENT_TYPE and existing_attachment_file_id:
            await message.answer(
                localize_text_for_user(
                    user_id,
                    "Bir nechta rasm saqlangan. Endi `Davom etish` tugmasini bosing yoki faqat rasm yuborishda davom eting.",
                ),
                reply_markup=build_attachment_upload_keyboard(get_user_script(user_id)),
                parse_mode="Markdown",
            )
    ...
    if not attachment_type or not attachment_file_id:
        prompt_message = await message.answer(
            localize_text_for_user(
                user_id,
                "Iltimos, qo'shimcha ma'lumot sifatida bir yoki bir nechta rasm, video yoki hujjat yuboring. \U0001F4CE",
            ),
            reply_markup=build_attachment_upload_keyboard(get_user_script(user_id)),
        )
    ...
    await show_section_text_prompt(
        ...
        section_key,
        user_id=user_id,
        intro_text=build_attachment_saved_notice(attachment_type, attachment_file_id),
    )
...
async def save_appeal_handler(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    request_id = save_request(
        ...
        phone=phone,
        username=username,
        message=build_message_with_attachment_note(appeal_text, attachment_type, attachment_file_id),
        attachment_type=attachment_type,
        attachment_file_id=attachment_file_id,
    )
    ...
    admin_text = (
        ...
        f"{build_attachment_admin_note(attachment_type, attachment_file_id, attachment_caption)}"
    )
...
async def main() -> None:
    ...
    try:
        # Polling mode uchun avval eski webhookni o'chirib yuboramiz.
